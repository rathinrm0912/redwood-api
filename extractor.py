...
# CORS FIX: allow_credentials=True + allow_origins=["*"] is invalid per CORS spec
# and causes "No Access-Control-Allow-Origin header" errors in the browser.
# PDF extraction doesn't need credentials — set allow_credentials=False.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Tries in order on rate-limit, 413, or decommission errors
GROQ_MODELS = [
    "llama-3.3-70b-versatile",  # Best quality — 100k TPD, 12k TPM
    "llama-3.1-8b-instant",     # Fast fallback — 500k TPD, high TPM
    "llama3-70b-8192",          # Emergency — separate quota pool
]
...
def groq_call_with_fallback(messages: list, max_tokens: int, label: str = "") -> dict:
    ...
    Falls through to next model on 429, 413, or decommissioned errors.
    On 413 (too large), trims user message by half before trying next model.
    ...
    for model in GROQ_MODELS:
        try:
            ...
            # Rate limit or decommissioned — try next model as-is
            if "429" in err_str or "rate_limit" in err_str or "decommissioned" in err_str:
                print(f"[Groq] {model} rate limited — trying next model")
                last_error = e
                continue

            # Request too large — trim user message by half then try next model
            elif "413" in err_str or "request too large" in err_str or "too large" in err_str:
                print(f"[Groq] {model} — request too large, trimming by half")
                last_error = e
                trimmed = []
                for msg in current_messages:
                    if msg["role"] == "user" and len(msg["content"]) > 2000:
                        half = len(msg["content"]) // 2
                        trimmed.append({"role": msg["role"], "content": msg["content"][:half]})
                    else:
                        trimmed.append(msg)
